...
def calcular_operacao(df: pd.DataFrame, preco_venda: np.ndarray,
    ...
    custo_aquisicao = (
        lance * (1 + cfg.comissao_leiloeiro + cfg.itbi + cfg.registro_cartorio)
        + cfg.custo_juridico_fixo
        + ocupado * cfg.custo_desocupacao
    )
    custo_reforma = np.where(df["tipo"] == "terreno", 0.0, area * cfg.reforma_por_m2)
    custo_posse = preco_venda * cfg.custo_mensal_posse * meses_ate_venda

    investimento_total = custo_aquisicao + custo_reforma + custo_posse
    ...
    return pd.DataFrame({
        ...
        "custo_reforma": custo_reforma.round(0),
